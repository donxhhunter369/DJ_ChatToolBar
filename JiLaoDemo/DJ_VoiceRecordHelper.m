...
#import "DJ_VoiceRecordHelper.h"
#import <AVFoundation/AVFoundation.h>

// Max record Time
#define kVoiceRecorderTotalTime 60.0

@interface DJ_VoiceRecordHelper()<AVAudioRecorderDelegate> {
    NSTimer *_timer;
...
@implementation DJ_VoiceRecordHelper
...
- (void)prepareRecordingWithPath:(NSString *)path prepareRecorderCompletion:(DJVoicePrepareRecorderCompletion)prepareRecorderCompletion {
    ...
    dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
        ...
        if(error) {
            DJLog(@"audioSession: %@ %ld %@", [error domain], (long)[error code], [[error userInfo] description]);
            return;
        }
...
- (void)getVoiceDuration:(NSString*)recordPath {
    ...
    if (error) {
        DJLog(@"recordPath：%@ error：%@", recordPath, error);
        self.recordDuration = @"";
    } else {
